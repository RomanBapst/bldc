...
void terminal_process_string(char *str) {
	...
	} else if (strcmp(argv[0], "print_conf") == 0) {
		commands_printf("mcconf.pwm_mode %.5f", (double)mcconf.pwm_mode);
		commands_printf("mcconf.comm_mode %.5f", (double)mcconf.comm_mode);
		commands_printf("mcconf.motor_type %.5f", (double)mcconf.motor_type);
		commands_printf("mcconf.sensor_mode %.5f", (double)mcconf.sensor_mode);
		commands_printf("mcconf.l_current_max %.5f", (double)mcconf.l_current_max);
		commands_printf("mcconf.l_current_min %.5f", (double)mcconf.l_current_min);
		commands_printf("mcconf.l_in_current_max %.5f", (double)mcconf.l_in_current_max);
		commands_printf("mcconf.l_in_current_min %.5f", (double)mcconf.l_in_current_min);
		commands_printf("mcconf.l_abs_current_max %.5f", (double)mcconf.l_abs_current_max);
		commands_printf("mcconf.l_min_erpm %.5f", (double)mcconf.l_min_erpm);
		commands_printf("mcconf.l_max_erpm %.5f", (double)mcconf.l_max_erpm);
		commands_printf("mcconf.l_erpm_start %.5f", (double)mcconf.l_erpm_start);
		commands_printf("mcconf.l_max_erpm_fbrake %.5f", (double)mcconf.l_max_erpm_fbrake);
		commands_printf("mcconf.l_max_erpm_fbrake_cc %.5f", (double)mcconf.l_max_erpm_fbrake_cc);
		commands_printf("mcconf.l_min_vin %.5f", (double)mcconf.l_min_vin);
		commands_printf("mcconf.l_max_vin %.5f", (double)mcconf.l_max_vin);
		commands_printf("mcconf.l_battery_cut_start %.5f", (double)mcconf.l_battery_cut_start);
		commands_printf("mcconf.l_battery_cut_end %.5f", (double)mcconf.l_battery_cut_end);
		commands_printf("mcconf.l_temp_fet_start %.5f", (double)mcconf.l_temp_fet_start);
		commands_printf("mcconf.l_temp_fet_end %.5f", (double)mcconf.l_temp_fet_end);
		commands_printf("mcconf.l_temp_motor_start %.5f", (double)mcconf.l_temp_motor_start);
		commands_printf("mcconf.l_temp_motor_end %.5f", (double)mcconf.l_temp_motor_end);
		commands_printf("mcconf.l_temp_accel_dec %.5f", (double)mcconf.l_temp_accel_dec);
		commands_printf("mcconf.l_min_duty %.5f", (double)mcconf.l_min_duty);
		commands_printf("mcconf.l_max_duty %.5f", (double)mcconf.l_max_duty);
		commands_printf("mcconf.l_watt_max %.5f", (double)mcconf.l_watt_max);
		commands_printf("mcconf.l_watt_min %.5f", (double)mcconf.l_watt_min);
		commands_printf("mcconf.lo_current_max %.5f", (double)mcconf.lo_current_max);
		commands_printf("mcconf.lo_current_min %.5f", (double)mcconf.lo_current_min);
		commands_printf("mcconf.lo_in_current_max %.5f", (double)mcconf.lo_in_current_max);
		commands_printf("mcconf.lo_in_current_min %.5f", (double)mcconf.lo_in_current_min);
		commands_printf("mcconf.lo_current_motor_max_now %.5f", (double)mcconf.lo_current_motor_max_now);
		commands_printf("mcconf.lo_current_motor_min_now %.5f", (double)mcconf.lo_current_motor_min_now);
		commands_printf("mcconf.sl_min_erpm %.5f", (double)mcconf.sl_min_erpm);
		commands_printf("mcconf.sl_min_erpm_cycle_int_limit %.5f", (double)mcconf.sl_min_erpm_cycle_int_limit);
		commands_printf("mcconf.sl_max_fullbreak_current_dir_change %.5f", (double)mcconf.sl_max_fullbreak_current_dir_change);
		commands_printf("mcconf.sl_cycle_int_limit %.5f", (double)mcconf.sl_cycle_int_limit);
		commands_printf("mcconf.sl_phase_advance_at_br %.5f", (double)mcconf.sl_phase_advance_at_br);
		commands_printf("mcconf.sl_cycle_int_rpm_br %.5f", (double)mcconf.sl_cycle_int_rpm_br);
		commands_printf("mcconf.sl_bemf_coupling_k %.5f", (double)mcconf.sl_bemf_coupling_k);
		commands_printf("mcconf.hall_sl_erpm %.5f", (double)mcconf.hall_sl_erpm);
		commands_printf("mcconf.foc_current_kp %.5f", (double)mcconf.foc_current_kp);
		commands_printf("mcconf.foc_current_ki %.5f", (double)mcconf.foc_current_ki);
		commands_printf("mcconf.foc_f_sw %.5f", (double)mcconf.foc_f_sw);
		commands_printf("mcconf.foc_dt_us %.5f", (double)mcconf.foc_dt_us);
		commands_printf("mcconf.foc_encoder_offset %.5f", (double)mcconf.foc_encoder_offset);
		commands_printf("mcconf.foc_encoder_ratio %.5f", (double)mcconf.foc_encoder_ratio);
		commands_printf("mcconf.foc_sensor_mode %.5f", (double)mcconf.foc_sensor_mode);
		commands_printf("mcconf.foc_pll_kp %.5f", (double)mcconf.foc_pll_kp);
		commands_printf("mcconf.foc_pll_ki %.5f", (double)mcconf.foc_pll_ki);
		commands_printf("mcconf.foc_motor_l %.5f", (double)mcconf.foc_motor_l);
		commands_printf("mcconf.foc_motor_r %.5f", (double)mcconf.foc_motor_r);
		commands_printf("mcconf.foc_motor_flux_linkage %.5f", (double)mcconf.foc_motor_flux_linkage);
		commands_printf("mcconf.foc_observer_gain %.5f", (double)mcconf.foc_observer_gain);
		commands_printf("mcconf.foc_observer_gain_slow %.5f", (double)mcconf.foc_observer_gain_slow);
		commands_printf("mcconf.foc_duty_dowmramp_kp %.5f", (double)mcconf.foc_duty_dowmramp_kp);
		commands_printf("mcconf.foc_duty_dowmramp_ki %.5f", (double)mcconf.foc_duty_dowmramp_ki);
		commands_printf("mcconf.foc_openloop_rpm %.5f", (double)mcconf.foc_openloop_rpm);
		commands_printf("mcconf.foc_sl_openloop_hyst %.5f", (double)mcconf.foc_sl_openloop_hyst);
		commands_printf("mcconf.foc_sl_openloop_time %.5f", (double)mcconf.foc_sl_openloop_time);
		commands_printf("mcconf.foc_sl_d_current_duty %.5f", (double)mcconf.foc_sl_d_current_duty);
		commands_printf("mcconf.foc_sl_d_current_factor %.5f", (double)mcconf.foc_sl_d_current_factor);
		commands_printf("mcconf.foc_sl_erpm %.5f", (double)mcconf.foc_sl_erpm);
		commands_printf("mcconf.foc_sample_v0_v7 %.5f", (double)mcconf.foc_sample_v0_v7);
		commands_printf("mcconf.foc_sample_high_current %.5f", (double)mcconf.foc_sample_high_current);
		commands_printf("mcconf.foc_sat_comp %.5f", (double)mcconf.foc_sat_comp);
		commands_printf("mcconf.foc_temp_comp %.5f", (double)mcconf.foc_temp_comp);
		commands_printf("mcconf.foc_temp_comp_base_temp %.5f", (double)mcconf.foc_temp_comp_base_temp);
		commands_printf("mcconf.s_pid_kp %.5f", (double)mcconf.s_pid_kp);
		commands_printf("mcconf.s_pid_ki %.5f", (double)mcconf.s_pid_ki);
		commands_printf("mcconf.s_pid_kd %.5f", (double)mcconf.s_pid_kd);
		commands_printf("mcconf.s_pid_min_erpm %.5f", (double)mcconf.s_pid_min_erpm);
		commands_printf("mcconf.s_pid_allow_braking %.5f", (double)mcconf.s_pid_allow_braking);
		commands_printf("mcconf.p_pid_kp %.5f", (double)mcconf.p_pid_kp);
		commands_printf("mcconf.p_pid_ki %.5f", (double)mcconf.p_pid_ki);
		commands_printf("mcconf.p_pid_kd %.5f", (double)mcconf.p_pid_kd);
		commands_printf("mcconf.p_pid_ang_div %.5f", (double)mcconf.p_pid_ang_div );
		commands_printf("mcconf.cc_startup_boost_duty %.5f", (double)mcconf.cc_startup_boost_duty);
		commands_printf("mcconf.cc_min_current %.5f", (double)mcconf.cc_min_current);
		commands_printf("mcconf.cc_gain %.5f", (double)mcconf.cc_gain);
		commands_printf("mcconf.cc_ramp_step_max %.5f", (double)mcconf.cc_ramp_step_max);
		commands_printf("mcconf.m_fault_stop_time_ms %.5f", (double)mcconf.m_fault_stop_time_ms);
		commands_printf("mcconf.m_duty_ramp_step %.5f", (double)mcconf.m_duty_ramp_step);
		commands_printf("mcconf.m_current_backoff_gain %.5f", (double)mcconf.m_current_backoff_gain);
		commands_printf("mcconf.m_encoder_counts %.5f", (double)mcconf.m_encoder_counts);
		commands_printf("mcconf.m_sensor_port_mode %.5f", (double)mcconf.m_sensor_port_mode);
		commands_printf("mcconf.m_invert_direction %.5f", (double)mcconf.m_invert_direction);
		commands_printf("mcconf.m_drv8301_oc_mode %.5f", (double)mcconf.m_drv8301_oc_mode);
		commands_printf("mcconf.m_drv8301_oc_adj %.5f", (double)mcconf.m_drv8301_oc_adj);
		commands_printf("mcconf.m_bldc_f_sw_min %.5f", (double)mcconf.m_bldc_f_sw_min);
		commands_printf("mcconf.m_bldc_f_sw_max %.5f", (double)mcconf.m_bldc_f_sw_max);
		commands_printf("mcconf.m_dc_f_sw %.5f", (double)mcconf.m_dc_f_sw);
		commands_printf("mcconf.m_ntc_motor_beta %.5f", (double)mcconf.m_ntc_motor_beta);
	} else if (strcmp(argv[0], "reset_fault") == 0) {
		DISABLE_GATE();
